...
func NewTransactionService(db *pgxpool.Pool, repo *repository.TransactionRepository, tpRepo *repository.TransactionProductRepository, cartRepo *repository.CartRepository) *TransactionService {
	return &TransactionService{
		db:                     db,
		repo:                   repo,
		transactionProductRepo: tpRepo,
		cartRepo:               cartRepo,
	}
}
...
func (s *TransactionService) Checkout(ctx context.Context, userID int, req models.CheckoutRequest) (*models.CheckoutResponse, error) {
	...
	for _, item := range req.Items {
		...
		if err := s.transactionProductRepo.CreateWithTx(ctx, tx, tp); err != nil {
